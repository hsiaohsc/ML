...
def regularized_linear_regression(X, y, lambd):
  ...
  len_D = len(X[0])
  XT = np.transpose(X)
  Identity = np.identity(len_D)
  lamdI = lambd * Identity
  X2 = XT @ X
  Xinverse = np.linalg.inv(X2+lamdI)
  w = Xinverse @ XT @ y
  #####################################################		 
  return w
...
def tune_lambda(Xtrain, ytrain, Xval, yval, lambds):
  ...
  for i in range(0,len(lambds)):
    lamdI = lambds[i] * Identity
    Xinverse = np.linalg.inv(X2+lamdI)
    w = Xinverse @ XT @ ytrain
    rsswn = 0
    for j in range(len(Xval)):
        fxn = Xval[j] @ np.transpose(w)
        rsswn += np.sum((fxn-yval[j])**2)
    rsswn/=len(Xval)
    if rsswn<rssw or i==0:
      rssw = rsswn
      index = i
  bestlambda = lambds[index]
  #####################################################		 
  return bestlambda
